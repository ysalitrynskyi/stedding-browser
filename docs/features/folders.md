...
Status: **F1–F12 built and tested**. F12 is round 7 (`docs/ARC-ROUND2.md`, 2026-09-05).
...
| F10 | Dragging a tab between two tabs inside an expanded folder places it there (the strip's own reorder is folder-aware). `MoveTabsToFolderAt` inserts at a child index; F7 still appends. | `TabStripModelTest.MoveTabsToFolderAtInsertsBetweenChildren` | built |
