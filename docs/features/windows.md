...
| N2 | The Windows build runs with the Stedding window: sidebar, Spaces, folders, the command bar, archive and routing; without branding, the macOS keyboard map, the Cocoa menus and the keychain naming, which live in `is_mac` sources. Not a port -- M8 -- but the first evidence for one. Branding followed as N3–N4, the keyboard map as N7. | none; live: `docs/images/win-wide.png` (the open sidebar) and `docs/images/win-rail.png` (the rail), Windows, 2026-09-08 | partial · M8, `S-56` |
...
| N5 | chrome://settings/stedding opens on Windows. The shortcut reference on that page is the Mac's keyboard map and its handler is Mac-only, and the page asked for it on every platform: a WebUI message nobody handles is a `NOTREACHED` in the browser process, so opening the Stedding settings killed the Windows browser until 2026-09-09. The page asks only where the reference exists (`steddingHasShortcutReference`); the reference reads the platform's own table since N7, so the block is on Windows too. | none; live: `w9_settings` (Windows, 2026-09-09) | partial · no unit test yet (S-49) |
| N6 | A per-user installer: Chromium's `mini_installer` from the `win-release` configuration (`tooling/args/win-release.gn`, `tooling/win/build.ps1`), packaged as `dist/Stedding-<VERSION>-win-x64.exe` with its sha256 (`tooling/win/package-installer.ps1`) and published beside the Mac's DMG by `tooling/publish-release` (ADR 0018). It installs to `%LOCALAPPDATA%\Stedding\Application` with the profile under `%LOCALAPPDATA%\Stedding\User Data`, registers the uninstall entry and the ProgIDs under HKCU, and `setup.exe --uninstall` removes all of it. Unsigned: SmartScreen warns, the notes say so. | none; live: an install and an uninstall on the build machine, the installed browser's About page `w9_installed_about` (Windows, 2026-09-09) | partial · no CI (S-49) |
| N7 | Stedding's keyboard on Windows is Arc's for Windows: the Mac's map with ⌘ read as Ctrl and ⌥⌘ as Ctrl+Alt (`chrome/browser/ui/accelerator_table.cc`, patch 0043). Ctrl+S collapses the sidebar (Save Page: Ctrl+Shift+S); Ctrl+D pins the tab in its Space and Ctrl+Shift+D shows or hides the address row (Bookmark This Tab and Bookmark All Tabs keep their menu rows, lose the keys); Alt+1–9 jump to a Space, Ctrl+Alt+←/→ step through them and Ctrl+Alt+Shift+←/→ move the tab across; Ctrl+Alt+↑/↓ walk the rows and Ctrl+Alt+Shift+↑/↓ move the row (Ctrl+PgUp/PgDn stay); Ctrl+Shift+K clears the Space; Ctrl+Shift+C copies the clean link and Ctrl+Alt+Shift+C the Markdown one (Inspect keeps F12 and Ctrl+Shift+I); Ctrl+Shift+2 / Ctrl+Alt+Shift+2 / Ctrl+Shift+1 are the screenshots; Ctrl+Shift+P opens the bar in actions mode (the system print dialog moves to Ctrl+Alt+P, and the print preview's link says so); Ctrl+Alt+Shift+N opens a Blank Window; Ctrl held a moment shows the row numbers (tabs R11), the Space chips keeping their glyphs. The Ctrl+Alt rows sit past Chromium's Ctrl+Alt check on purpose (HANDOFF trap 42); the tab-group chords are Linux's alone (menus M6). Little windows have no chords here yet (S-56). | `ShortcutReferenceTest.*` (every Stedding command resolves through the platform's table; on Windows the palette's divergence row and the Alt note); live: `w10_shortcuts` (the block on chrome://settings/stedding), `w10_tabmenu` (Ctrl+D on the pin verb), Windows, 2026-09-10 | partial · the tests are written, not yet run on a Windows runner (S-49) |
