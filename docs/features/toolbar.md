...
| T14 | The bottom-left downloads button shows Chromium's progress ring and the download-started animation; the toolbar's own button never appears at the top right during a download (backlog S-41). | live: `tooling/drive` starts a download, a capture within 1 s shows the ring on the sidebar button and no toolbar button; dark and light | partial · `DownloadToolbarUIController` draws the ring and the started animation on the sidebar button when the Stedding anchor exists (patch 0021); the download capture is still owed |
