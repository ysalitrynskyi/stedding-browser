...
| Q10 | No field-trial testing config: `disable_fieldtrial_testing_config = true` in every build configuration (`tooling/args/*.gn`). Chromium activates `testing/variations/fieldtrial_testing_config.json` -- its own experiment set, hundreds of features switched on by a file in the tree -- in every build that does not set it, official builds included, so every beta up to 4 ran with it; on Windows it put up the "launch when Windows starts" infobar (2026-09-09). Nothing contacts a server for this; it is about whose defaults the browser runs. The seven features of Chromium's 2026 desktop refresh that the config had switched on -- MenuSimplification, RoundedIcons, TabGroupColorRefresh, TabStripDeclutter, ToolbarGlowUp, UpdateTabGroupColors, WebuiRefresh2026 -- are on by default in the source instead (patch 0042), since every round was measured against that look; the first build without them drew Chromium's old logo in the address chip. S-57 reviews each on its merits. The config had also been hiding a fill: with it off, every inactive sidebar row and essentials card took Chromium's Material inactive-tab colour, which no beta had shown; the Stedding mixer pins that colour transparent (tabs R23, the Mac pass of 2026-09-10). | none; live: the installed Windows browser without the infobar (`w9_installed_about`, 2026-09-09); the Mac's beta 5 build carries the same argument | partial · no unit test yet (S-49) |
