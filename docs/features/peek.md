...
throttle list for renderer-initiated main-frame navigations in pinned tabs) for links that
navigate the tab, and one check at the top of `chrome::Navigate()` for links that ask for a
new tab. The overlay is
