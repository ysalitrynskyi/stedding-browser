...
Status: **T1 built**; **T2–T4 planned**. Owner: ADR 0019 follow-on (in-memory jars). Patch: 0048.
...
|---|---|---|---|
| T1 | "Open in Throwaway" creates an isolated Space with `StoragePartitionConfig` `in_memory=true`. | `SpaceWindowTest.ThrowawayTabUsesAnInMemoryPartition`; `SpaceRegistryTest.ThrowawayIsInMemoryAndIsolated` | built |
