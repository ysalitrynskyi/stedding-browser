...
| I24 | The keychain item Stedding stores its own key under is named for Stedding ("Stedding Safe Storage" / "Stedding"), so macOS's own panels name the browser the user is running; a profile from an earlier beta still decrypts, because a missing item falls back to the Chromium one it was written under and nothing is written back. | `KeychainPasswordTest.SteddingNamesTheItemAndFallsBackToChromium`; live: the keychain panel during an import names Stedding | built |
| I25 | A profile written by an earlier beta keeps reading the Chromium-named keychain item forever: nothing rewrites its key under Stedding's own name, so deleting that item (or a real Chromium on the same Mac rotating it) leaves the profile's saved passwords undecryptable. A one-time re-encrypt on first run under the new name would close it (`BACKLOG.md` S-51). | none | gap |
