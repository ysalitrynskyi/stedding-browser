...
|---|---|---|---|
| H1 | Both pin tiers record a home URL at pin time: SpaceModel::SetSpacePinned(tab, true) stores handle → GURL (space_pinned_ becomes a map), and Chromium's pinned bit flipping on (OnTabStripModelChanged) records it for essentials. | `SpaceWindowTest.PinRecordsHomeUrl` | built |
...
| H3 | ⌘W (IDC_CLOSE_TAB) on a Space-pinned or essentials tab sleeps it instead of closing (item 7's look; activation moves to the next visible row in the Space); a sleeping pinned tab is left alone; removal is Unpin. | `SpaceWindowTest.CloseOnPinnedTabSleepsIt` (the sleep and the activation move; the no-op on an already-sleeping pin is not asserted yet) | built |
...
| H7 | Peek reads the stored home URL's eTLD+1 as the pinned site (P2), so a drifted pin stops peeking the wrong site. | `SpaceWindowTest.PinRecordsHomeUrl` (its last assertion is `stedding::PinnedSiteFor` returning the stored home URL, which is what peek reads) | built |
...
| H9 | Setting off returns ⌘W to closing (Unpin implied). | `SpaceWindowTest.CloseOnPinnedTabSleepsIt` (its second half sets `kPinsCloseSleeps` false and expects `CloseAction::kClose`) | built |
