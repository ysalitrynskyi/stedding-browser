...
| Y7 | Collapsed with the row hidden, the rail stays at the window's top (Chromium dropped it a caption's height down, which sat the toggle at 57 DIP) and its toggle clears the traffic lights by their own height and a 6 DIP gap, so it sits just under them; on the way out of expand-on-hover it drops below them instead of passing under them. | `VerticalTabStripTopContainerTest.CollapsedButtonClearsTheCaptionButtonsByTheirOwnHeight`, `VerticalTabStripTopContainerTest.LeavingExpandOnHoverNeverSitsOnTheCaptionRow`; live: `r7_collapsed` | built |
