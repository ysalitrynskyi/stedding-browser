...
Status: **B1–B17 built and tested** (patches 0004, 0005, 0013); **B18–B31 built** (round 6, `docs/ROUND6-PLAN.md` R6-02, R6-18; patches 0016–0021, 0025); **B32 built** (round 10, patch 0052).
...
| B10 | Per-Space pin: a tab pinned in its Space sits above the Clear line there and survives Clear. | `SpaceWindowTest.CmdDTogglesSpacePin` (it sits at the top of the unpinned block), `SpaceWindowTest.ClearUnpinnedTabsKeepsPins` (it survives Clear) | built |
...
| B32 | Changing Space slides the sidebar's list: the new Space's rows come in from the side of the Space being entered -- from the right for the next Space, from the left for the previous one, whatever changed it (the switcher, ⌃1–9, ⌥⌘←/→, the swipe) -- over 200 ms with an ease-out, the title row and the essentials above it staying still. No slide under `stedding::ShouldAnimate()` false (motion O1). Arc animates the change; Stedding swapped the rows in one frame, which the operator read as "no animation for scroll between spaces" (2026-09-24, round 10). | `SpaceSlideTest.ComesInFromTheSideOfTheSpaceBeingEntered` (the direction); the slide itself is a layer animation, seen, not asserted | built |
