...
| N2 | The Windows build runs with the Stedding window: sidebar, Spaces, folders, the command bar, archive and routing; without branding, the macOS keyboard map, the Cocoa menus and the keychain naming, which live in `is_mac` sources. Not a port -- M8 -- but the first evidence for one. | none; live: `w8_base` (Windows, 2026-09-08) | partial · M8 |
